...
<pre><code>
# <h2 id="install"></a>POPULATION GENOMICS ANALYSIS FOR CLONAL ORGANISMS</h2>
...
<pre><code id="code-install">required_packages <- c("poppr", "phangorn", "RClone", "radiator", "dartR", "vegan", "ape", "ggplot2", "conStruct", "pheatmap", 
...
<button onclick="copyToClipboard('code-install')">Copy Code</button>


# <h2 id="file-setup"></a>STEP 1: FILE LOADING AND INITIAL SETUP</h2>
# Load your dataset (handle .gen, .vcf, or GenAlEx files).

<pre><code id="code-file-setup">x <- getfile()  # Optional interactive file selection
x
...
<button onclick="copyToClipboard('code-file-setup')">Copy Code</button>
...
# <h2 id="filtering"></a>STEP 2. DATA FILTERING AND CLEANUP</h2>
# Apply initial filtering of loci and individuals with missing data and remove uninformative loci.
<pre><code id="code-filtering">filtered_data_loci <- missingno(genomic_data, "loci")  # Remove loci with >5% missing values
...
<button onclick="copyToClipboard('code-filtering')">Copy Code</button>


# <h2 id="modify-population"></a>STEP 3. MODIFY POPULATION NAMES AND REORDER POPULATIONS</h2>
# Modify population names based on your dataset. You can customize the population names here.
<pre><code id="code-modify-population">pop(filtered_data_loci_inform_indiv)[pop(filtered_data_loci_inform_indiv) == "Pop1"] <- "Location_A"
...
<button onclick="copyToClipboard('code-modify-population')">Copy Code</button>
...
#<h2 id="analysis-with-replicates"></a>Module 1 uses genets, ramets and technical replicates for genetic distance estimation and create cut off values and get Mlg Ids. Once this created, the technical replicates need to be removed</h2> 
<pre><code id="code-analysis-with-replicates"># MODULE 1: Clonality Estimation
...
<button onclick="copyToClipboard('code-analysis-with-replicates')">Copy Code</button>


# ============================================
# <h2 id="analysis-without-replicates"></a>MODULE 4 ONWARDS: ANALYSIS WITHOUT TECHNICAL REPLICATES (ONLY ONE INDIVIDUAL PER MLG, PER LOCALITY)</h2>
...
<pre><code id="code-analysis-without-replicates">filtered_data_MLG_corrected <- clonecorrect(filtered_data_loci_inform_indiv, strata = ~Pop)
...
# <h2 id="pcoa"></a>MODULE 4: Principal Coordinate Analysis (PCoA) WITHOUT TECHNICAL REPLICATES</h2>
...
<button onclick="copyToClipboard('code-analysis-without-replicates')">Copy Code</button>


# <h2 id="phylogram"></a>MODULE 5: Phylogram Creation (OPTIONAL)</h2>
<pre><code id="code-phylogram"># Phylograms for populations (without individual resolution)
phylogram_nj <- gl.tree.nj(genomic_data_genlight, type = "phylogram")
plot(phylogram_nj)
</code></pre>
<button onclick="copyToClipboard('code-phylogram')">Copy Code</button>


# <h2 id="ancestry"></a>MODULE 6: Ancestry Analysis Using LEA</h2>
<pre><code id="code-ancestry"># Convert dataset to structure format for LEA
...
<button onclick="copyToClipboard('code-ancestry')">Copy Code</button>

# <h2 id="local-adaptation"></a>MODULE 7: Local Adaptation with pcadapt</h2>
...
<pre><code id="code-local-adaptation"># STEP 1: Remove uninformative loci and filter individuals with high missing data
...
<button onclick="copyToClipboard('code-local-adaptation')">Copy Code</button>



# <h2 id="visualization"></a>MODULE 8: Visualization of Genetic Relationships</h2>
<pre><code id="code-visualization"># Create a heatmap of genetic distances
...
<button onclick="copyToClipboard('code-visualization')">Copy Code</button>
